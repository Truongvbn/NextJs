...
import { Users, Star, Mail, Book, ShoppingCart, Heart, Clock, ExternalLink, ChevronDown, ChevronUp, Eye } from 'lucide-react';
...
const StatCard = ({ title, value, icon, color }) => {
  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-gray-800/50' : 'bg-white/50'} backdrop-blur-md rounded-xl overflow-hidden shadow-lg p-6 h-full transform transition duration-500 hover:scale-105 border border-gray-200 dark:border-gray-700`}>
      <div className="flex justify-between items-center h-full relative">
        <div className="z-10">
          <h3 className="text-lg font-semibold mb-2 line-clamp-2 h-12 text-gray-800 dark:text-white" title={title}>{title}</h3>
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{value}</p>
        </div>
        <div className={`absolute right-0 top-0 w-32 h-32 ${color} opacity-10 rounded-full -mr-16 -mt-16`}></div>
        <div className="z-10">
...
const InstructorCard = ({ instructor }) => {
  ...
  return (
    <div className={`group h-full flex flex-col rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl ${
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
          <h3 className="text-xl font-bold text-white mb-1 truncate" title={instructor.name || "Unknown Instructor"}>
            {instructor.name || "Unknown Instructor"}
          </h3>
          <p className="text-sm text-gray-300 truncate" title={instructor.sum || "Expert Instructor"}>
      ...
      <div className="flex-grow p-6 flex flex-col">
        ...
        <div className={`overflow-hidden transition-all duration-300 ${isExpanded ? 'max-h-40' : 'max-h-0'}`}>
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-4" title={instructor.description || "No description available."}>
  ...
  return (
    <div className={`group h-full flex flex-col rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl ${
      ...
      <div className="p-4 flex-grow flex flex-col justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white line-clamp-2 mb-2" title={course.title || "Untitled Course"}>
            {course.title || "Untitled Course"}
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-2 mb-3" title={course.sum || "No summary available."}>

...
import { Bell, MessageSquare, Search, Moon, Sun, User, Menu, ChevronDown, Book, Settings, LogOut } from 'lucide-react';
...
const Header = ({ toggleSidebar }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 bg-white/60 dark:bg-gray-800/60 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-700/50 h-16 flex items-center justify-between px-4 z-50 transition-all duration-300 ${
      scrollPosition > 50 ? 'shadow-md' : ''
    }`}>
      <div className="flex items-center space-x-4">
        <button onClick={toggleSidebar} className="p-2 rounded-md hover:bg-gray-100/50 dark:hover:bg-gray-700/50 transition-colors duration-200">
      ...
      <div className="flex items-center space-x-4">
        ...
        <button className="relative p-2 rounded-full bg-gray-100/50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-600/50 transition-colors duration-200 group">
          ...
          <span className="absolute top-full mt-1 right-0 bg-white dark:bg-gray-800 text-sm py-1 px-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200">Notifications</span>
        </button>
        <button className="relative p-2 rounded-full bg-gray-100/50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-600/50 transition-colors duration-200 group">
          <MessageSquare size={20} />
          <span className="absolute top-full mt-1 right-0 bg-white dark:bg-gray-800 text-sm py-1 px-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200">Messages</span>
        </button>
        <button
        ...
        >
          ...
          <span className="absolute top-full mt-1 right-0 bg-white dark:bg-gray-800 text-sm py-1 px-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200">Toggle Theme</span>
        </button>
        <div className="relative">
          ...
          {isProfileDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md rounded-md shadow-lg py-1 z-10">
              <a href="#" className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100/50 dark:hover:bg-gray-700/50 transition-colors duration-200 flex items-center">
